...
def calculate_annualized_roi(purchase_price, current_value, purchase_date):
    ...
    if days_held < 1:
        return calculate_total_roi(
            purchase_price, current_value
        )  # Not held long enough for annualization
...
def process_asset_performance(asset, fx_rates):
    ...
    if asset.type.lower() == "stock":
        try:
            ...
            if (
                ...
                and asset.purchase_price > 0
                and fx_rates
            ):
                current_fx_rate = (
                    fx_rates.get(asset.currency, 1.0)
                ...
                currency_gain_usd = (
                    (current_fx_rate - (asset.purchase_fx_rate or 1.0))
                    * quantity
                    * asset.purchase_price
                )

                dividend_income_usd = (
                    sum(d.amount for d in asset.dividends) * current_fx_rate
                )
    ...
    return {
        ...
        "purchase_date": (
            asset.purchase_date.strftime("%Y-%m-%d") if asset.purchase_date else None
        ),
        ...
        "dividends": [
            {"amount": d.amount, "date": d.date.isoformat(), "projected": d.projected}
            for d in asset.dividends
        ],
    }
