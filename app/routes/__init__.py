...
def get_properties():
    ...
    for prop in properties:
        result.append({
            ...
            'roi': calculate_annualized_roi(prop.purchase_price, prop.current_value, prop.purchase_date)
        })
    return jsonify(result)
...
def upload_stocks_csv():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'message': f'{len(csv_input)} assets processed successfully.'}), 201
